...
afl_2025 <- data.frame(
  Date = afl_data$utcStartTime,
  Season = year,  # Using the provided year parameter
  Team = afl_data$home.team.name,
  Opponent = afl_data$away.team.name,
  status = afl_data$status,
  Home = TRUE,
  ELO = NA,
  Opp_ELO = NA
)
...
# Apply the latest ELO ratings to the 2025 fixture and calculate ELO difference
apply_elo_to_fixture <- function(afl_2025, latest_elos) {
  for (i in 1:nrow(afl_2025)) {
    ...
    # Calculate ELO difference (Team ELO - Opponent ELO)
    afl_2025$Elo_Difference[i] <- afl_2025$ELO[i] - afl_2025$Opp_ELO[i]
  }
...
# Betting -- Not very accurate 
afl_2025$home_odds_glm <- 1 / afl_2025$win_prob_glm_1
afl_2025$away_odds_glm <- 1 / (1 - afl_2025$win_prob_glm_1)

# View sorted by GLM win probability
View(afl_2025 %>% arrange(('utcStartTime')))
